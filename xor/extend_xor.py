"""
Extend (add layers) and continue training an existing model
"""
...
with tf.name_scope('layer3_2'):
    W3_2 = tf.Variable(tf.random_uniform([number_hidden_nodes,number_hidden_nodes], -.1, .1), name='w3_2')
    b3_2 = tf.Variable(tf.random_uniform([number_hidden_nodes], -.01, .01), name='b3_2')
    hidden3_2 = tf.nn.relu(tf.matmul(hidden2, W3_2) + b3_2)
    hidden3_2 = tf.identity(hidden2, name="hidden3_2")

# Fourth layer
with tf.name_scope('layer4'):
    W4 = tf.Variable(tf.random_uniform([number_hidden_nodes,2], -.1, .1), name='w3')
    b4 = tf.Variable(tf.zeros([2]), name='b4')
    hidden4 = tf.matmul(hidden3_2, W4) + b4
    hidden4 = tf.identity(hidden4, name="hidden4")

y_2 = tf.nn.softmax(hidden4, name="y_2")
cross_entropy_2 = -tf.reduce_sum(outputs*tf.log(y_2), name="cross_entropy_2")
train_step = tf.train.GradientDescentOptimizer(0.2).minimize(cross_entropy_2)
tf.summary.scalar("cross_entropy_2", cross_entropy_2)
...
with tf.Session() as sess:
    writer = tf.summary.FileWriter('logs/'+str(run_number)+'_2/', sess.graph)
    ...
    for step in range(5000):
        ...
        e,a,summary=sess.run([cross_entropy_2, train_step, merged],feed_dict)
        # if e<1:break # early stopping yay
        print("step %d : entropy %s" % (step,e)) # error/loss should decrease over time
        # write log
        writer.add_summary(summary, step)

    saver.save(sess, "checkpoints/"+str(run_number)+"_2/xor.ckpt") # save data

    # Test trained model
    correct_prediction = tf.equal(tf.argmax(y_2,1), tf.argmax(outputs,1)) # argmax along dim-1
    ...
    learned_output=tf.argmax(y_2,1)
